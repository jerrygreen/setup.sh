...
const envPath = execSync(`nu -c $nu.env-path`).toString().trim()
const configPath = execSync(`nu -c $nu.config-path`).toString().trim()
...
for (let f of RC_FILES) {
  ...
  promises.push(
    fetch(url)
      .then((r) => r.text())
      .then((content) => fs.writeFileSync(file, content, { encoding: 'utf-8', flag: 'w' }))
  )
}

if (!env.includes(envCmd)) fs.appendFileSync(envPath, '\n' + envCmd + '\n')
if (!config.includes(configCmd)) fs.appendFileSync(configPath, '\n' + configCmd + '\n')
